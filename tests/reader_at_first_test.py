...
class TestReader:
    ...
    async def test_inventory(self):
        """ test inventory
        """
        await self._reader.set_antenna(1)
        # settings = await self._reader.get_inventory_settings()
        await self._reader.set_inventory_settings(phase=True)
        ...
        for tag in inventory:
            print(json.dumps(tag))
        await self._reader.set_inventory_settings(phase=False)
        inventory = await self._reader.get_inventory()
        for tag in inventory:
            print(json.dumps(tag))
